...
- ✅ **Backup & Restore + SyncIndicator** (Feb 2026): admin-only `/api/admin/backups*` — daily auto snapshots (asyncio scheduler, default 02:00 UTC, retention 365), manual/import/restore with safety `before_restore_*` snapshot, audit log, "delete last backup" guard, /admin/backup UI page with CountChips per collection. Persistent TopBar `SyncIndicator` dot (green/red/blue + pending count). 16/16 backend pytest + frontend e2e all green.
- ✅ **Offline Login** (Feb 2026): device-bound credential cache in IndexedDB `mosaytra-auth` — PBKDF2-SHA256 (100k iters, 128-bit per-credential random salt). Password NEVER persisted. First login on a device MUST be online; after that, the same device can log in offline. Admin-side account disable / password rotation propagates: on `online` events and after every successful sync drain, `/auth/me` is re-verified → 401/403 burns the local credential and forces re-login. Friendly Arabic errors for the two failure modes (wrong-password vs first-online-required). 9/9 spec items + 2 regressions pass.
